...
def test_get_name_from_id(mock_app_lookup):
    ...
    assert AppType.get_name_from_id("mod1.app1", None) == "App 1"
    assert AppType.get_name_from_id("mod1/app1", None) == "App 1"
    assert AppType.get_name_from_id("not_real", None) is None


def test_get_name_from_id_fail(mock_network_error):
    with pytest.raises(EntityNameError) as e:
        AppType.get_name_from_id("foo", None)
...
def test_get_names_from_ids(mock_app_lookup):
    ...
    names = AppType.get_names_from_ids(app_list, None)
    for app in app_list:
        assert app in names
...
def test_get_names_from_ids_fail(mock_network_error):
    with pytest.raises(EntityNameError) as e:
        AppType.get_names_from_ids(["app1", "app2"], None)
    assert "Unable to find app names" in str(e)


def test_validate_id(mock_app_lookup):
    mock_app_lookup({"mod1/app1": "App 1"})
    assert AppType.validate_id("mod1/app1", None)
    assert AppType.validate_id("mod1.app1", None)
    assert AppType.validate_id("not_an_app", None) is False


def test_validate_id_fail(mock_network_error):
    assert AppType.validate_id("Some_app", None) is False
